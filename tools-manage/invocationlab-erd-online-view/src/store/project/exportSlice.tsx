...
const ExportSlice = (set: SetState<ProjectState>, get: GetState<ProjectState>) => ({
  ...
  exportFile: (type: string) => {
    ...
    if (type === 'Markdown') {
      ...
      const postfix = type === 'Word' ? '.docx' : '.pdf';
      // 保存图片
      get().dispatch.showExportMessage();
      saveImage(dataSource, columnOrder, (images: any) => {
        ...
        request.post('/ncnb/project/exportErd', {
          method: 'POST',
          responseType: 'blob',
          timeout: 300000,
          data: {
      ...
      File.save(secret, `${project}.erd.aes.txt`);
    } else if (type === 'JSON') {
      let tempDataSource = { ...dataSource }
      const originERDJson = JSON.stringify(tempDataSource, null, 2);
      File.save(originERDJson, `${project}.erd.json`);
    }
  },
  showExportMessage: () => {
    ...
    if (allTable.length > 50) {
      Modal.warning({
        title: '导出提示',
        content: `当前导出的数据表较多， 共【${allTable.length}】张表，请耐心等待，导出完毕之前请勿关闭此窗口，导出完成后会弹出下载窗口！`,
        okText: null,
        cancelText: null,
      });
    } else {
      Modal.warning({
        title: '导出提示',
        content: `正在导出， 共【${allTable.length}】张表，请耐心等待，导出完毕之前请勿关闭此窗口，导出完成后会弹出下载窗口！`,
        okText: null,
        cancelText: null,
      });
    }
